...
{
cur=first;
//printf("\n before while"); while(cur->link!=first) cur=cur->link;
//printf("\n after while"); cur->link=temp;
temp->link=first;
}
return first;
}
...
if(second == NULL) return first; cur=first;
while(cur->link!=first) cur=cur->link;
cur->link = second; do{
cur=cur->link;
}while(cur->link!=second); cur->link=first;
return first;
}
void main()
{
NODE first = NULL, second = NULL; int choice;
printf("\n********* Creation of CSLL - 1 and CSLL - 2 *********\n"); while(1)
{
printf("\n1. Insert to CSLL - 1\n2. Insert to CSLL - 2\n3. Display CSLL - 1\n4. Display CSLL - 2\n5. concatenate the CSLLs\n6. Exit\n");
printf("\n	\n");
...
case 5: printf("\n Linked lists will be concatenated now ");
first = concat(first,second); printf("\nCSLLs concatenated ");
display(first); second=NULL; break;
case 6: printf("\n... CSLL Concatenation program will be terminated now ");
exit(0);
